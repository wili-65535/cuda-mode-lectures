...
__global__ void vectorAdd(const float *a, const float *b, float *result,
                          int n) {
...
__global__ void vectorAddPrivatized(const float *a, const float *b,
                                    float *result, int n) {
  int index = threadIdx.x + blockIdx.x * blockDim.x;
  if (index < n) {
...
int main() {
  ...
  vectorAddPrivatized<<<blocksPerGrid, threadsPerBlock>>>(d_a, d_b, d_result,
                                                          n);
