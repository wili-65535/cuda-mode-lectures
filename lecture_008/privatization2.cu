...
__global__ void windowSumDirect(const float *input, float *output, int n,
                                int windowSize) {
...
__global__ void windowSumPrivatized(const float *input, float *output, int n,
                                    int windowSize) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  int halfWindow = windowSize / 2;
  __shared__ float sharedData[1024]; // Assuming blockDim.x <= 1024

  // Load input into shared memory (for demonstration, assuming window fits into
  // shared memory)
  if (idx < n) {
    ...
    for (int i = -halfWindow; i <= halfWindow; ++i) {
      ...
      if (accessIdx >= 0 && accessIdx < blockDim.x && (idx + i) < n &&
          (idx + i) >= 0) {
...
int main() {
  ...
  // Execute kernels
  windowSumDirect<<<blocksPerGrid, threadsPerBlock>>>(d_input, d_output, n,
                                                      windowSize);
  cudaMemcpy(output, d_output, n * sizeof(float),
             cudaMemcpyDeviceToHost); // Copy result back for verification

  windowSumPrivatized<<<blocksPerGrid, threadsPerBlock>>>(d_input, d_output, n,
                                                          windowSize);
  cudaMemcpy(output, d_output, n * sizeof(float),
             cudaMemcpyDeviceToHost); // Copy result back for verification
