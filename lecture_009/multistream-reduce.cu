...
int main() {
  ...
  for (int i = 0; i < NUM_DEVICES; ++i) {
    cudaSetDevice(i);
    cudaMemcpyAsync(d_input[i], h_input, bytes, cudaMemcpyHostToDevice,
                    streams[i]);
  }

  // Launch the kernel with coarsening on each device
  int numBlocks =
      (size + BLOCK_DIM * COARSE_FACTOR - 1) / (BLOCK_DIM * COARSE_FACTOR);
  for (int i = 0; i < NUM_DEVICES; ++i) {
    cudaSetDevice(i);
    CoarsenedReduction<<<numBlocks, BLOCK_DIM, 0, streams[i]>>>(
        d_input[i], d_output[i], size);
  }
  ...
  for (int i = 0; i < NUM_DEVICES; ++i) {
    ...
    cudaSetDevice(i);
    cudaMemcpyAsync(d_output_host[i], d_output[i], sizeof(float),
