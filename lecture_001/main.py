...
module = torch.utils.cpp_extension.load_inline(name='square',
                                               cpp_sources='',
...
def square(input):
    ...
    blocks_per_grid = (input.numel() +
                       (threads_per_block - 1)) // threads_per_block
    module.square_kernel(blocks_per_grid, threads_per_block, input, output,
                         input.numel())
    return output
