...
__global__ void mean_filter_kernel(unsigned char *output, unsigned char *input,
...
torch::Tensor mean_filter(torch::Tensor image, int radius) {
  ...
  mean_filter_kernel<<<number_of_blocks, threads_per_block, 0,
                       torch::cuda::getCurrentCUDAStream()>>>(
      result.data_ptr<unsigned char>(), image.data_ptr<unsigned char>(), width,
      height, radius);
