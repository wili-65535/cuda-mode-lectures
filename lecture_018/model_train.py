...
def main(config: str, use_torch_compile: bool):
    ...
    dlrm = DLRM(metadata=metadata, parameters=model_parameters, device=hyperparameters['device']).to(hyperparameters['device'])
    ...
    valid_loader = iter(DataLoader(valid_dataset, batch_size=batch_size_valid, shuffle=False))
    ...
    writer = SummaryWriter(log_dir=hyperparameters["tensorboard_dir"], flush_secs=30, filename_suffix=modifier)
    ...
    prof = torch.profiler.profile(
        ...
        on_trace_ready=torch.profiler.tensorboard_trace_handler(hyperparameters["tensorboard_dir"], worker_name=modifier),
        record_shapes=True,
        profile_memory=True,
        with_stack=True
    )
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx in tqdm(range(hyperparameters['batches_per_epoch']), ncols=80):
            ...
            writer.add_scalar("Accuracy/train", correct_predictions / total_predictions, index)
