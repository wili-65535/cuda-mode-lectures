...
class MLP(nn.Module):
    ...
    def __init__(self, input_size):
        ...
        self.layers = nn.Sequential(nn.Linear(input_size, input_size // 2), nn.ReLU(), nn.Linear(input_size // 2, input_size // 4), nn.Sigmoid())
...
class LinearWithLoRA(torch.nn.Module):
    ...
    def __init__(self, linear, rank, alpha, device):
        super().__init__()
        self.linear = linear
        self.lora = LoRALayer(linear.in_features, linear.out_features, rank, alpha, device=device)
