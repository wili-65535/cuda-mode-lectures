...
@triton.jit
def fused_add_mul_relu(in_out_ptr0, in_ptr0, in_ptr1, xnumel,
...
@triton.jit
def fused_add_mul_relu_cleaner(dense_in_out_ptr, scalar_ptr, dense_ptr,
                               num_weights, xnumel, multiplier,
                               BLOCK_SIZE: tl.constexpr):
    xoffset = tl.program_id(0) * BLOCK_SIZE
    ...
    tmp1 = tl.load(scalar_ptr + scalar_index,
                   mask,
...
def fused_add_mul_relu_torch(in_out_tensor: torch.Tensor, bias: torch.Tensor,
...
def fused_add_mul_relu_cleaner_torch(in_out_tensor: torch.Tensor,
                                     bias: torch.Tensor,
                                     in_tensor: torch.Tensor) -> torch.Tensor:
    # print("calling fused_add_mul_relu_torch")
    grid = lambda meta: (triton.cdiv(in_out_tensor.numel(), meta['BLOCK_SIZE']
...
def get_inputs(batch_size: int = 8,
               weight_size: int = 8,
               add_manual_seed: bool = False):
    if add_manual_seed:
        torch.manual_seed(0)
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        x_names=['batch_size', 'weight_size'
                 ],  # Argument names to use as an x-axis for the plot.
        x_vals=[(2**i, 2**j)
                for i, j in zip(range(2, 20, 2), range(2, 11, 1))],
        # Different possible values for `x_name`.
        x_log=True,  # x axis is logarithmic.
        line_arg=
        'provider',  # Argument name whose value corresponds to a different line in the plot.
        line_vals=['torch.compile.generated',
                   'cleaner'],  # Possible values for `line_arg`.
        line_names=['torch.compile.generated',
                    'cleaner'],  # Label name for the lines.
        styles=[('blue', '-'), ('green', '-')],  # Line styles.
        ylabel='GB/s',  # Label name for the y-axis.
        plot_name=
        'performance',  # Name for the plot. Used also as a file name for saving the plot.
        args={},  # Values for function arguments not in `x_names` and `y_name`.
    ))
def benchmark(batch_size, weight_size, provider):
    in_out_tensor, in_tensor, bias = get_inputs(batch_size=batch_size,
                                                weight_size=weight_size,
                                                add_manual_seed=True)
    quantiles = [0.5, 0.2, 0.8]
    if provider == "torch.compile.generated":
        ms, min_ms, max_ms = triton.testing.do_bench(
            lambda: fused_add_mul_relu_torch(in_out_tensor, bias, in_tensor),
            quantiles=quantiles)
    if provider == "cleaner":
        ms, min_ms, max_ms = triton.testing.do_bench(
            lambda: fused_add_mul_relu_cleaner_torch(in_out_tensor, bias,
                                                     in_tensor),
            quantiles=quantiles)
...
if __name__ == '__main__':
    ...
    expected_output = torch.maximum(in_out_tensor + 0.5 * in_tensor + bias,
    ...
    torch.testing.assert_close(in_out_tensor,
                               expected_output,
                               rtol=1e-4,
                               atol=1e-4)
