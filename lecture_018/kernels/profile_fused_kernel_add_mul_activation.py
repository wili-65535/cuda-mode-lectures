import torch
from triton_fused_add_mul_activation import (fused_add_mul_activation_torch,
                                             get_inputs)

if __name__ == '__main__':
    sample_inputs = [
        get_inputs(batch_size=65336, weight_size=512, add_manual_seed=False)
        for _ in range(10)
    ]
    BLOCK_SIZE = 1024
    prof = torch.profiler.profile(
        ...
        on_trace_ready=torch.profiler.tensorboard_trace_handler(
            "kernel_benchmarks", worker_name="triton_vs_torchscript"),
        profile_memory=True,
        with_stack=True)
    prof.start()
