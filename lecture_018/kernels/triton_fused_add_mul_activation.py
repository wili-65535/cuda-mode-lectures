...
@triton.jit
def fused_add_mul_activation_kernel(x_ptr, bias_ptr, in_ptr, num_weights: tl.constexpr, xnumel: tl.constexpr, multiplier: tl.constexpr, activation: tl.constexpr, BLOCK_SIZE: tl.constexpr):
...
def fused_add_mul_activation_torch(in_out_tensor: torch.Tensor, bias: torch.Tensor, in_tensor: torch.Tensor) -> torch.Tensor:
    ...
    fused_add_mul_activation_kernel[grid](in_out_tensor, bias, in_tensor, bias.numel(), in_out_tensor.numel(), multiplier=0.5, activation="sigmoid", BLOCK_SIZE=BLOCK_SIZE)
...
def benchmark(batch_size, weight_size, provider):
    ...
    if provider == "triton":
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: fused_add_mul_activation_torch(in_out_tensor, bias, in_tensor), quantiles=quantiles)
    if provider == "torch":
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: add_mul_activation_torch(in_out_tensor, bias, in_tensor), quantiles=quantiles)

    if provider == "torch_scripted":
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: add_mul_activation_torch_scripted(in_out_tensor, bias, in_tensor), quantiles=quantiles)
