...
class DenseArch(nn.Module):
    ...
    def __init__(self, dense_feature_count: int, dense_hidden_layers_sizes: List[int], output_size: int, *args, **kwargs) -> None:
        super(DenseArch, self).__init__(*args, **kwargs)
        self.mlp = MLP(input_size=dense_feature_count, hidden_sizes=dense_hidden_layers_sizes, output_size=output_size)  # D X O
...
class SparseArch(nn.Module):
    ...
    def __init__(self, metadata: Dict[str, Union[int, List[int], Dict[str, int]]], embedding_sizes: Mapping[str, int], device: str = "cpu", *args, **kwargs) -> None:
        ...
        self.sparse_layers = nn.ModuleList([SparseFeatureLayer(cardinality=self._modulus_hash_sizes[i], embedding_size=embedding_sizes[feature_name]) for i, feature_name in enumerate(metadata.keys())])
        ...
        self.mapping = [torch.tensor(metadata[f"SPARSE_{i}"]["tokenizer_values"], device=device) for i in range(self.num_sparse_features)]
...
class PredictionLayer(nn.Module):
    ...
    def __init__(self, dense_out_size: int, sparse_out_sizes: List[int], hidden_sizes: List[int], *wargs, **kwargs):
        super(PredictionLayer, self).__init__(*wargs, **kwargs)
        concat_size = sum(sparse_out_sizes) + dense_out_size
        self.mlp = MLP(input_size=concat_size * concat_size, hidden_sizes=hidden_sizes, output_size=1)
...
class DLRM(nn.Module):
    ...
    def __init__(self, metadata: Dict[str, Union[int, List[int]]], parameters: Parameters, device: str = "cpu"):
        super(DLRM, self).__init__()
        self.dense_layer = DenseArch(dense_feature_count=parameters.dense_input_feature_size, dense_hidden_layers_sizes=parameters.dense_mlp["hidden_layer_sizes"], output_size=parameters.dense_mlp["output_size"])
        self.sparse_layer = SparseArch(metadata=metadata, embedding_sizes=parameters.sparse_embedding_sizes, device=device)
        self.interaction_layer = DenseSparseInteractionLayer()
        self.prediction_layer = PredictionLayer(
            dense_out_size=parameters.dense_mlp["output_size"],
            sparse_out_sizes=[parameters.sparse_embedding_sizes[f"SPARSE_{i}"] for i in range(len(parameters.sparse_embedding_sizes))],
...
@click.command()
@click.option('--file_path', type=click.Path(exists=True), help='Path to the parquet file', default="data/sample_criteo_data.parquet")
@click.option('--metadata_path', type=click.Path(exists=True), help='Path to the metadata file', default="data/sample_criteo_metadata.json")
def dry_run_with_data(file_path, metadata_path):
    ...
    dense_arch = DenseArch(dense_feature_count=num_dense_features, dense_hidden_layers_sizes=[32], output_size=dense_mlp_out_size)
    ...
    sparse_arch = SparseArch(metadata=metadata, embedding_sizes=embedding_sizes)
    ...
    prediction_layer = PredictionLayer(dense_out_size=dense_mlp_out_size, sparse_out_sizes=[sparse_mlp_out_size] * len(metadata), hidden_sizes=[16])
    ...
    parameters = Parameters(dense_input_feature_size=13, sparse_embedding_sizes={"SPARSE_{}".format(i): 16 for i in range(26)}, dense_mlp={"hidden_layer_sizes": [16], "output_size": 16}, prediction_hidden_sizes=[16], use_modulus_hash=True)
    import torch._dynamo
    torch._dynamo.reset()
